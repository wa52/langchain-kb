...
- [x] `src/cli/console.py` main loop distinguishes `_agent_result is an Exception` from still-loading; failed → echo `[错误] 知识库加载失败…` (reuse existing failure copy); bare Enter no longer loops "正在加载中"
- [x] `tests/test_console.py` adds a `TestLoadingState`-style case: after load failure, plain input and empty input both yield the failure message (not "正在加载中"), and `/help` still works
- [x] `python -m pytest tests/test_console.py -v` green
