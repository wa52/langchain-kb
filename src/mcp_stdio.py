...
def answer_with_knowledge(query: str, session_id: str | None = None) -> dict:
    ...
    _ensure_ready()
    from src.api.services.chat import chat_with_rag, extract_sources
...
async def start_index_task(path: str, exclude: list[str] | None = None) -> dict:
    ...
    _ensure_ready()
    from src.api.services.indexing import run_index_task
    mgr = _task_manager()
...
def list_sessions() -> dict:
    ...
    from src.agent.chat_history import list_sessions
...
def set_graph_extraction_mode(enabled: bool) -> dict:
    ...
    from src.api.services.settings import set_graph_extraction_mode as _svc
    return _svc(enabled)
